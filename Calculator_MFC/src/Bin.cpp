...
std::string Bin::ExpBinToDec(std::string exp){      
    std::string number_str;
    int lp {0};
    std::string result;
    std::string check ="01+-*/()";

    for (int i {0}; i < exp.size(); i++){       //sprawdzenie czy jest wprowadzony odpowiedni format
    ...
    for (int i {0}; i < exp.size(); i++){
        if(isdigit(exp[i])){        //wyodrębnienie liczby binarnej
            ...
            for (int j {0}; j < number_str.size(); j++){
                lp += (number_str[number_str.size()-1-j] - '0')*(pow(2,j));     //zamiana wyodrębnionej liczby binarnej na liczbe dziesiętną
            }
